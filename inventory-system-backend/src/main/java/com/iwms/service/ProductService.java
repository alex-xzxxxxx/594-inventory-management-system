...
public class ProductService {
    ...
    public Product create(Product p){p.setId(store.nextProductId()); store.products.put(p.getId(),p); store.inventory.put(p.getId(),new com.iwms.model.InventoryItem(p.getId(),0,5)); return p;}
    ...
    public boolean delete(Long id){if(!store.products.containsKey(id))return false;store.products.remove(id);store.inventory.remove(id);return true;}
}
